...
def get_verification_email_msg(verification_instance, as_html=False):
    ...
    if as_html:
        return f"<h1>Verify your email with following <a href='{verify_link}'>here</a></h1>"
    return f"Verify your email with following {verify_link}"
...
def verify_token(token, max_attempts=5):
    ...
    if max_attempts_reached.exists():
        return False, "Token expired, used too many time", None
    obj = qs.first()
    obj.attempts += 1
